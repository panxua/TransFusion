...
class TransFusionHead(nn.Module):
    def __init__(self,
        ...
        self.img_guidance = self.fuse_img and img_guidance
        self.fuse_img_decoder = self.fuse_img and fuse_img_decoder
        self.fuse_fov = self.fuse_img and fuse_fov
        self.fuse_bev = self.fuse_img and fuse_bev
        assert not self.fuse_img or (self.fuse_fov or self.fuse_bev)
        self.fuse_bev_collapse = self.fuse_bev and fuse_bev_collapse
        self.shared_decoder = self.fuse_bev and shared_decoder

        self.order = []
        for view_type in order:
            if view_type=='bev' and self.fuse_bev:
                self.order.append('bev')
            if view_type=='fov' and self.fuse_fov:
                self.order.append('fov')

        if self.fuse_img:
            self.num_views = num_views
            self.out_size_factor_img = out_size_factor_img
            self.shared_conv_img = nn.ModuleDict({"fov":
                build_conv_layer(
                dict(type='Conv2d'),
            ...
            if self.initialize_by_heatmap:
                self.heatmap_head_img =  nn.ModuleDict({"fov":copy.deepcopy(self.heatmap_head), "bev":copy.deepcopy(self.heatmap_head)})
            if self.fuse_img_decoder:
                self.img_decoder, self.img_heads = nn.ModuleDict({"fov":nn.ModuleList(), "bev":nn.ModuleList()}), nn.ModuleDict({"fov":nn.ModuleList(), "bev":nn.ModuleList()})
                if not shared_decoder:
                    count = 0
                    for view_type in self.order:
                        if (view_type == 'fov' and self.fuse_fov) or (view_type == 'bev' and self.fuse_bev):
                            count += 1
                            # transformer decoder layers/prediction for fov fusion
                            for i in range(self.num_fusion_layers):
                                self.img_decoder[view_type].append(
                                    TransformerDecoderLayer(
                                        hidden_channel, num_heads, ffn_channel, dropout, activation,
                                        self_posembed=PositionEmbeddingLearned(2, hidden_channel),
                                        cross_posembed=PositionEmbeddingLearned(2, hidden_channel),
                                ...
                                heads = copy.deepcopy(common_heads)
                                heads.update(dict(heatmap=(self.num_classes, num_heatmap_convs)))
                                self.img_heads[view_type].append(FFN(hidden_channel * (1+count), heads, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias))
                else:
                    self.shared_img_decoder = nn.ModuleList()
                    for i in range(self.num_fusion_layers):
                        self.shared_img_decoder.append(TransformerDecoderLayer(
                                        hidden_channel, num_heads, ffn_channel, dropout, activation,
                                        self_posembed=PositionEmbeddingLearned(2, hidden_channel),
                                        cross_posembed=PositionEmbeddingLearned(2, hidden_channel),
                                    ))
            ...
            self.fc = nn.ModuleDict({"fov":nn.Sequential(*[nn.Conv1d(hidden_channel, hidden_channel, kernel_size=1)])})
            if not self.fuse_bev or self.fuse_bev_collapse:       
                self.fc['bev'] = nn.Sequential(*[nn.Conv1d(hidden_channel, hidden_channel, kernel_size=1)])
            else:
                self.downsample = [nn.Conv2d(hidden_channel, hidden_channel, 3, padding=1, bias=False),
                ...
                self.downsample_list = nn.Sequential(*(self.downsample+copy.deepcopy(self.downsample)))
                # self.downsample_list = nn.Sequential(*(self.downsample+copy.deepcopy(self.downsample)+copy.deepcopy(self.downsample)))
        self.init_weights()
    ...
    def init_weights(self):
        ...
        if self.fuse_img:
            if self.fuse_bev:
                for m in self.img_decoder['bev'].parameters():
                    if m.dim() > 1:
                        nn.init.xavier_uniform_(m)

                if not self.fuse_bev_collapse:
                    for m in self.downsample_list.modules():
                        if isinstance(m, nn.Conv2d):
                            kaiming_init(m)

                for m in self.shared_conv_img['bev'].modules():
                    if isinstance(m, nn.Conv2d):
                        kaiming_init(m)
                # for decoder in self.bev_decoder:
                #     decoder.init_weights()
                for ffn in self.img_heads['bev']:
                    ffn.init_weights()
            ...
            if self.fuse_fov:
                ...
                for m in self.shared_conv_img['fov'].modules():
                    if isinstance(m, nn.Conv2d):
                        kaiming_init(m)
                # for decoder in self.fov_decoder:
                #     decoder.init_weights()
                for ffn in self.img_heads['fov']:
                    ffn.init_weights()
    ...
    def feat_to_BEV(self, feat_flatten, feat_pos, view_type, img_inputs, img_metas):
        img_feat = self.shared_conv_img[view_type](img_inputs)  # [BS * n_views, C, H, W]

        img_h, img_w, num_channel = img_inputs.shape[-2], img_inputs.shape[-1], img_feat.shape[1]
        raw_img_feat = img_feat.view(self.batch_size, self.num_views, num_channel, img_h, img_w).permute(0, 2, 3, 1, 4) # [BS, C, H, n_views, W]
        img_feat = raw_img_feat.reshape(self.batch_size, num_channel, img_h, img_w * self.num_views)  # [BS, C, H, n_views*W]
        
        assert view_type in ['fov','bev']
        if view_type=="fov" or (view_type=="bev" and self.fuse_bev_collapse):
            img_feat_collapsed = img_feat.max(2).values
            img_feat_collapsed = self.fc[view_type](img_feat_collapsed).view(self.batch_size, num_channel, img_w * self.num_views)

            # positional encoding for image guided query initialization
            if self.img_feat_collapsed_pos[view_type] is None:
            ...
            bev_feat = feat_flatten
            assert len(self.img_decoder[view_type])==self.num_fusion_layers*(not self.shared_decoder)+self.num_views
            if view_type == "fov":
                for idx_view in range(self.num_views):
                    bev_feat = self.img_decoder[view_type][self.num_fusion_layers*(not self.shared_decoder) + idx_view](bev_feat, img_feat_collapsed[..., img_w * idx_view:img_w * (idx_view + 1)], feat_pos, img_feat_collapsed_pos[:, img_w * idx_view:img_w * (idx_view + 1)])
            if view_type == "bev":
                bev_feat = self.img_decoder[view_type][self.num_fusion_layers*(not self.shared_decoder) + 1](bev_feat, img_feat_collapsed[..., img_w * idx_view:img_w * (idx_view + 1)], feat_pos, img_feat_collapsed_pos[:, img_w * idx_view:img_w * (idx_view + 1)])
        elif view_type=="bev":
            img_feat_shrink = self.downsample_list(img_feat)
            ...
            img_feat_shrink = img_feat_shrink.view(self.batch_size, num_channel, -1)
            bev_feat = feat_flatten
            assert len(self.img_decoder[view_type])==self.num_fusion_layers*(not self.shared_decoder)+1
            bev_feat = self.img_decoder[view_type][self.num_fusion_layers*(not self.shared_decoder)](bev_feat, img_feat_shrink, feat_pos, img_feat_shrink_pos)
    ...
    def forward_single(self, inputs, fov_inputs, bev_inputs, img_metas):
        ...
        # image to BEV
        #################################
        lidar_feat_flatten = lidar_feat.view(self.batch_size, lidar_feat.shape[1], -1)  # [BS, C, H*W]
        bev_pos = self.lidar_bev_pos.repeat(self.batch_size, 1, 1).to(lidar_feat.device)

        if self.fuse_img:
            img_feat, img_inputs = {}, {}
            if self.fuse_bev:
                img_inputs['bev'] = bev_inputs
                img_feat['bev'] = self.shared_conv_img['bev'](bev_inputs)  # [BS * n_views, C, H, W]
            if self.fuse_fov:
                img_inputs['fov'] = fov_inputs
                img_feat['fov'] = self.shared_conv_img['fov'](fov_inputs)  # [BS * n_views, C, H, W]


            #TODO repeat
            raw_img_feat, bev_feat = {},{}
            for view_type in self.order:
                img_h, img_w, num_channel = img_inputs[view_type].shape[-2], img_inputs[view_type].shape[-1], img_feat[view_type].shape[1]
                raw_img_feat[view_type] = img_feat[view_type].view(self.batch_size, self.num_views, num_channel, img_h, img_w).permute(0, 2, 3, 1, 4) # [BS, C, H, n_views, W]
                bev_feat[view_type] = self.feat_to_BEV(lidar_feat_flatten, bev_pos, view_type, img_inputs[view_type], img_metas)
        ...
        if self.initialize_by_heatmap:
            ...
            #TODO bev_feat is a dict
            if self.img_guidance:
                for view_type in self.order: #FOV, BEV
                    dense_heatmap_img[view_type] = self.heatmap_head_img[view_type](bev_feat[view_type].view(lidar_feat.shape))  # [BS, num_classes, H, W]
                heatmap_img = torch.mean(torch.cat([val.sigmoid()[None,:] for val in dense_heatmap_img.values()],dim=0),dim=0)
                heatmap = (dense_heatmap.detach().sigmoid() + heatmap_img.detach().sigmoid()) / 2
            else:
            ...
            query_pos = self.query_pos.repeat(self.batch_size, 1, 1).to(lidar_feat.device)  # [BS, num_proposals, 2]
        ...
        if self.fuse_img_decoder:
            ...
            prev_query_feat = {}
            query_feats = [query_feat.detach().clone()] #lidar query feats


            if not self.shared_decoder:
                for v_id,view_type in enumerate(self.order):
                    if view_type not in raw_img_feat:
                        continue
                    query_feats.append(query_feat.detach().clone())
                    # initialize
                    self.on_the_image_mask[view_type] = torch.BoolTensor(self.batch_size,0).cuda()

                    # positional encoding for image fusion
                    #TODO raw_img_feat is a dict
                    img_feat = raw_img_feat[view_type].permute(0, 3, 1, 2, 4) # [BS, n_views, C, H, W]
                    img_feat_flatten = img_feat.view(self.batch_size, self.num_views, num_channel, -1)  # [BS, n_views, C, H*W]
                    if self.img_feat_pos[view_type] is None:
                        (h, w) = img_inputs[view_type].shape[-2], img_inputs[view_type].shape[-1]
                        img_feat_pos = self.img_feat_pos[view_type] = self.create_2D_grid(h, w).to(img_feat_flatten.device)
                    else:
                        img_feat_pos = self.img_feat_pos[view_type]

                    # modified: add a for-loop
                    lidar_query_feat = query_feat.detach().clone()
                    for i in range(self.num_fusion_layers):
                        prev_query_feat[view_type] = query_feat.detach().clone()
                        query_feat = torch.zeros_like(query_feat)
                        # store prev_query_feat as feature source
                        # to refine query_feat again and again
                        ...
                        on_the_image_mask = torch.ones([self.batch_size, self.num_proposals]).to(query_pos_3d.device) * -1
                        ...
                        for sample_idx in range(self.batch_size):
                            ...
                            if self.batch_size == 1:  # skip during inference to save time
                            ...
                            for view_idx in range(self.num_views):
                                ...
                                query_feat_view = prev_query_feat[view_type][sample_idx, :, on_the_image]
                                query_pos_view = torch.cat([center_xs, center_ys], dim=-1)
                                query_feat_view = self.img_decoder[view_type][i](query_feat_view[None], img_feat_flatten[sample_idx:sample_idx + 1, view_idx], query_pos_view[None], img_feat_pos, attn_mask=attn_mask.log())
                                nan_mask = torch.isnan(query_feat_view)
                                query_feat[sample_idx, :, on_the_image][~nan_mask.squeeze()] = query_feat_view.clone()[[~nan_mask]]
                                query_feats[1+v_id] = query_feat.detach().clone()
            
                    if self.auxiliary:
                        self.on_the_image_mask[view_type] = torch.cat([self.on_the_image_mask[view_type],(on_the_image_mask != -1)],1)
                    else:
                        self.on_the_image_mask[view_type] = (on_the_image_mask != -1)

                    res_layer = self.img_heads[view_type][i](torch.cat(query_feats, dim=1))
                    res_layer['center'] = res_layer['center'] + query_pos.permute(0, 2, 1)
                    for key, value in res_layer.items():
                        ...
                        res_layer[key][~self.on_the_image_mask[view_type][:,-self.num_proposals:].unsqueeze(1).repeat(1, pred_dim, 1)] = \
                            first_res_layer[key][~self.on_the_image_mask[view_type][:,-self.num_proposals:].unsqueeze(1).repeat(1, pred_dim, 1)]
                    res_layer['view_type'] = view_type
                    ret_dicts.append(res_layer)
                    # for next level positional embedding
                    query_pos = res_layer['center'].detach().clone().permute(0, 2, 1)
            else:
            ...
            #             img_feat_flatten_list.append(prev_query_feat[view_type][sample_idx, :, on_the_image])
            #             img_feat_pos_list.append(torch.cat([center_xs, center_ys], dim=-1))
            #             fused_mask.append(attn_mask)

            #         img_feat_flatten = torch.cat(img_feat_flatten_list,dim=-1)
            #         img_feat_pos = torch.cat(img_feat_pos_list,dim=1)
            #         attn_mask = torch.cat(fused_mask,dim=0)

            #         query_feat_view = prev_query_feat[view_type][sample_idx, :, on_the_image]
            #         query_pos_view = torch.cat([center_xs, center_ys], dim=-1)
            #         query_feat_view = self.shared_img_decoder[view_type][i](query_feat_view[None], img_feat_flatten[sample_idx:sample_idx + 1, view_idx], query_pos_view[None], img_feat_pos, attn_mask=attn_mask.log())
            #         nan_mask = torch.isnan(query_feat_view)
            #         query_feat[sample_idx, :, on_the_image][~nan_mask.squeeze()] = query_feat_view.clone()[[~nan_mask]]
            #         query_feats[1+v_id] = query_feat.detach().clone()
        ...
        if self.initialize_by_heatmap:
            ...
            if self.img_guidance:
                ret_dicts[0]['dense_heatmap'] = list(dense_heatmap_img.values())
            else:
        ...
        for key in ret_dicts[0].keys():
            if key not in ['dense_heatmap', 'dense_heatmap_old', 'query_heatmap_score', 'view_type']:
                ...
                if key is 'view_type':
                    new_res[key] = [res_dict[key] for res_dict in ret_dicts]
                else:
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        if hasattr(self, 'on_the_image_mask'):
            temp_count = {'bev':0,'fov':0}
            assert len(preds_dicts[0]) == 1
            for i, view_type in enumerate(preds_dicts[0][0]['view_type']):
                label_weights[:,i*self.num_proposals:(i+1)*self.num_proposals] = label_weights[:,i*self.num_proposals:(i+1)*self.num_proposals] * self.on_the_image_mask[view_type][:,temp_count[view_type]*self.num_proposals:(temp_count[view_type]+1)*self.num_proposals]
                bbox_weights[:,i*self.num_proposals:(i+1)*self.num_proposals] = bbox_weights[:,i*self.num_proposals:(i+1)*self.num_proposals] * self.on_the_image_mask[view_type][:,temp_count[view_type]*self.num_proposals:(temp_count[view_type]+1)*self.num_proposals,None]
                temp_count[view_type] += 1
        ...
        if self.auxiliary:
            num_layers = self.num_decoder_layers if not self.fuse_img_decoder else self.num_fusion_layers*(self.fuse_bev+self.fuse_fov)
        else:
            num_layers = 1
