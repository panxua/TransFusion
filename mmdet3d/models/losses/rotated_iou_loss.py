...
def rotated_iou_3d_loss(pred, target):
    ...
    Args:
        pred (torch.Tensor): Bbox predictions with shape [B, N, 7]
            (x, y, z, w, l, h, alpha).
        target (torch.Tensor): Bbox targets (gt) with shape [B, N, 7]
            (x, y, z, w, l, h, alpha).

    Returns:
        torch.Tensor: IoU loss between predictions and targets.
    """
    iou_loss = 1 - diff_iou_rotated_3d(pred,
                                       target) #[0] modified to multi-batch
    return iou_loss
