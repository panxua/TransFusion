...
from .vod_data_utils import get_vod_image_info


def convert_to_vod_info_version2(info):
    """convert vod info v1 to v2 if possible.

    Args:
        info (dict): Info of the input vod data.
...
def _calculate_num_points_in_gt(data_path,
    ...
    for info in mmcv.track_iter_progress(infos):
        ...
        # annos = vod.filter_vod_anno(annos, ['DontCare'])
...
def create_vod_info_file(data_path,
                           ...
                           save_path=None,
                           relative_path=True):
    """Create info file of vod dataset.
    ...
    vod_infos_train = get_vod_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, vod_infos_train, relative_path, num_features=7)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'vod info train file is saved to {filename}')
    mmcv.dump(vod_infos_train, filename)
    vod_infos_val = get_vod_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
        ...
        relative_path=relative_path)
    _calculate_num_points_in_gt(data_path, vod_infos_val, relative_path, num_features=7)
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'vod info val file is saved to {filename}')
    mmcv.dump(vod_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'vod info trainval file is saved to {filename}')
    mmcv.dump(vod_infos_train + vod_infos_val, filename)

    vod_infos_test = get_vod_image_info(
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
    ...
    print(f'vod info test file is saved to {filename}')
